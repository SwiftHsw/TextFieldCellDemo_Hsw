...
- (void)setStatus:(NSString *)string {
    ...
    if(string) {
       // CGSize stringSize = [string sizeWithFont:self.stringLabel.font constrainedToSize:CGSizeMake(200, 300)];
        ...
        CGSize aSize=CGSizeMake(200, 300);
        
        CGRect aRect=[string boundingRectWithSize:aSize options:(NSStringDrawingUsesFontLeading|NSStringDrawingTruncatesLastVisibleLine|NSStringDrawingUsesLineFragmentOrigin) attributes:@{NSFontAttributeName: self.stringLabel.font} context:nil];
        
        stringWidth = aRect.size.width;
        stringHeight = aRect.size.height;
...
- (void)showWithStatus:(NSString*)string maskType:(SVProgressHUDMaskType)hudMaskType networkIndicator:(BOOL)show {
    if(!self.superview){
        [self.overlayWindow setRootViewController:[[UIViewController alloc]init]];
